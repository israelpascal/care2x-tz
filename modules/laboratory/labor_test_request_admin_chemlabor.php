...
/* Here begins the real work */
	...
	if(!$mode) /* Get the pending test requests */ 	{
		...
		$sql_headline="SELECT care_person.pid, care_person.selian_pid,care_encounter.current_room_nr,name_first,name_2,name_last, sex, batch_nr, 
...
tr.encounter_nr,tr.send_date,dept_nr,room_nr,tr.create_id FROM care_test_request_".$subtarget." tr, care_encounter, care_person
		...
		if($h_requests=$db->Execute($sql_headline)){
			if ($test_request_headline = $h_requests->FetchRow()) {
				...
				$h_room_nr=$test_request_headline['current_room_nr'];
				...
				$h_name_2=$test_request_headline['name_2'];
...
function printOut()
{
	urlholder="labor_test_request_printpop.php?sid=<?php echo $sid ?>&lang=<?php echo $lang ?>&user_origin=<?php echo $user_origin ?>&target=<?php echo $target ?>&subtarget=<?php echo $subtarget ?>&batch_nr=<?php echo $batch_nr ?>&pn=<?php echo $stored_request['encounter_nr'] ?>&h_selian_file_number=<?php echo $h_selian_file_number?>&h_name_last=<?php echo $h_name_last?>&h_name_first=<?php echo $h_name_first ?>&h_name_2=<?php echo $h_name_2 ?>&h_birthdate=<?php echo $h_birthdate;?>&h_sex=<?php echo $h_sex ?>&h_DoctorID=<?php echo $h_DoctorID?>&h_encounter_class_nr=<?php echo $h_encounter_class_nr?>&h_room_nr=<?php echo $h_room_nr?>&roomprefix=<?php echo $roomprefix?>&h_ipd_admission=<?php echo $h_ipd_admission ?>&h_opd_admission=<?php echo $h_opd_admission ?>";

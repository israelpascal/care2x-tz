...
<table cellspacing="0"  class="titlebar" border=0>
 <tr valign=top  class="titlebar" >
  ...
  <td bgcolor="{{$top_bgcolor}}" align=right>{{if !$disableButton}}{{if $pbAux2}}<a
   ...
   href="javascript:history.go(-1)"><img {{$gifBack2}} alt="" {{$dhtml}} ></a>{{/if}}{{if $pbHelp}}<a
   href="{{$pbHelp}}"><img {{$gifHilfeR}} alt="" {{$dhtml}}></a>{{/if}}{{if $breakfile}}<a
   href="javascript:window.close()" {{$sCloseTarget}}><img {{$gifClose2}} alt="{{$LDCloseAlt}}" {{$dhtml}}></a>{{/if}}{{/if}}
   {{if $disableButton}}<a
